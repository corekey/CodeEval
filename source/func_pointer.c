 ...
 * ---------------------
 * Description: Defect code used to identify invalid function pointers
 *
 ***********************************************************************/
...
/***********************************************************************
 *
 * Defect type: Invalid function pointer 
 * Complexity: Use of function pointer with mismatched return type 
 *
 ***********************************************************************/
...
void func_pointer_001 ()
{
	...
	ret = func(); /* ERROR: Bad function pointer casting */
}
...
void func_pointer_002 ()
{
	...
	fptr(buf[0]); /* ERROR: Bad function pointer casting */
}
...
void func_pointer_003 ()
{
	...
	if(1)
	{
		...
		ret = func(1, 2); /* ERROR: Bad function pointer casting */
	}
}
...
void func_pointer_004 ()
{
    	...
    	for(j = 0; j <= 1; j++)
    	{
         	char str;
            	...
            	str = fptr(buf[j]); /* ERROR: Bad function pointer casting */
        }
}
 ...
 *
 ***********************************************************************/
long **  func_pointer_005_func_001()
{
	int i,j;
	long ** doubleptr;
...
void func_pointer_005()
{
	...
	{
		void (*fptr)(void);
	    	fptr = (void (*)(void ))func_pointer_005_func_001;
	    	fptr(); /* ERROR: Bad function pointer casting */
	    	for(i=0;i<10;i++)
	    	{
	    		for(j=0;j<10;j++)
		    	{
...
void func_pointer_006()
{
	...
	if(flag==0)
	{
		...
		char **doubleptr;
	    	...
	    	doubleptr = fptr(); /* ERROR: Bad function pointer casting */
...
void func_pointer_007 ()
{
	...
	a =fptr(buf); /* ERROR: Bad function pointer casting */
}
 ...
 *
 ***********************************************************************/
static float a[2][3] = { {1.0,2.0,3.0}, {11.1,22.1,33.1} };
...
void func_pointer_008 ()
{
	...
	switch(1)
	{
		case 1:
		{
			...
			ptr = fptr(a,1); /* ERROR: Bad function pointer casting */
			printf("%f \n",**ptr);
			break;
		}
		default:
...
void func_pointer_009 ()
{
	...
	*p = fptr(); /* ERROR: Bad function pointer casting */
	ret = p->b;
	free(p);
	p= NULL;
}
...
void func_pointer_010 ()
{
	...
	func1 = func;
	ret = func1(); /* ERROR: Bad function pointer casting */
}
...
void func_pointer_011 ()
{
    	...
    	for(j = 0; j <= 1; j++)
    	{
			...
			fptr1(buf[j]); /* ERROR: Bad function pointer casting */
       	 	}
    	}
 }
...
void func_pointer_012 ()
{
	...
	ptr = (int (*)[]) func(); /* ERROR: Bad function pointer casting */
	free(ptr);
}
...
my_label:
    	if (flag == 1)
	{
        	...
        	func_gbl(1); /* ERROR: Bad function pointer casting */
	}
	ptr = &arr[0];
	*(ptr+1) = 7;
	ret ++;
...
my_label2:
 	...
 	{
        	float f;
    		f = func_gbl(); /* ERROR: Bad function pointer casting */
  	}
	return ret;
}
...
void func_pointer_015_func_004(func_pointer_015_s_001* st1)
{
	...
	else
	{
		...
		st = fptr_gbl( st1); /* ERROR: Bad function pointer casting */
	}
}
