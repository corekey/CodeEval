 ...
 * ---------------------
 * Description: Defect code used to identify access to freed memory 
 *
 ***********************************************************************/
...
/***********************************************************************
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after being freed when using conditional operator 
 *
 ***********************************************************************/
void invalid_memory_access_001 ()
{
	...
	int flag=10;
    	...
    	if (flag == 10)
	{
    		a = *(ptr+1); /* ERROR: Invalid memory access to already freed area */
	}
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed in else clause after being freed 
 *
 ***********************************************************************/
void invalid_memory_access_002 ()
{
	...
	{
    		a = *(dptr+1); /* ERROR: Invalid memory access to already freed area */
		printf("%lf",a);
	}
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after free for use in assignment 
 *
 ***********************************************************************/
void invalid_memory_access_003 ()
{
	...
	while(i>0)
	{
		...
		if (buf!=NULL)
		{
	    	...
	    	*c = *buf; /* ERROR: Invalid memory access to already freed area */
	    	i++;
	    	if (i>=10)
		{
	    		break;
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after free for use in printf function 
 *
 ***********************************************************************/
void invalid_memory_access_004 ()
{
	...
	if (staticflag1)
    	...
    	if (staticflag1)
    	{
        	printf("String= %s",buf); /* ERROR: Invalid memory access to already freed area */
    	}
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after free in recursive function 
 *
 ***********************************************************************/
int invalid_memory_access_005 (int count)
{
	...
	if (count ==0)
	{
    		count = *ptr; /* ERROR: Invalid memory access to already freed area */
		return 1;
	}
    	else
    	{
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after free when using a 2D array 
 *
 ***********************************************************************/
void invalid_memory_access_006 ()
{
	...
	free(buf4);
	free(buf5);
	*((*pbuf[1])+1) =buf2[0]; /* ERROR: Invalid memory access to already freed area */
}

/***********************************************************************
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after free for use as argument to strcpy function 
 *
 ***********************************************************************/
void invalid_memory_access_007 ()
{
	...
	free(ptr);
	strcpy(*(ptr+2),"String"); /* ERROR: Invalid memory access to already freed area */
}

/***********************************************************************
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Memory is accessed after free for use as argument to memcpy function 
 *
 ***********************************************************************/
void invalid_memory_access_008 ()
{
	...
	{
		free(buf);
		memcpy(buf,buf1,11); /* ERROR: Invalid memory access to already freed area */
	}
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Union member is accessed after alias to memory is freed 
 *
 ***********************************************************************/
typedef union {
	int *a;
...
void invalid_memory_access_009 ()
{
	...
	invalid_memory_access_009_uni_001 *p  = NULL;
		...
		free(u->s1);
		free(u);
		p->s1->a[0] = 1; /* ERROR: Invalid memory access to already freed area */
    	}
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Freed memory allocated in another function then used after being freed 
 *
 ***********************************************************************/
...
void invalid_memory_access_010 ()
{
	...
	free(ptr1);
	*(ptr1+1) = ptr[1]; /* ERROR: Invalid memory access to already freed area */
}

/***********************************************************************
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Array element is accessed after being freed 
 *
 ***********************************************************************/
void invalid_memory_access_011 ()
{
	...
	*(ptr+i) = i; /* ERROR: Invalid memory access to already freed area */
}

/***********************************************************************
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Freed memory is used after switch statement 
 *
 ***********************************************************************/
typedef struct {
	int a;
...
int invalid_memory_access_012_func_001 (int flag)
{
	...
	return (i+s->a); /* ERROR: Invalid memory access to already freed area */
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Double pointer alias is used to write to memory after memory was freed 
 *
 ***********************************************************************/
typedef struct {
	int a;
...
int invalid_memory_access_013_func_002 (int flag)
{
	...
	return invalid_memory_access_013_s_001_s_gbl->a; /* ERROR: Invalid memory access to already freed area */
}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Freed memory used after goto statement  
 *
 ***********************************************************************/
int invalid_memory_access_014 (int flag)
{
	...
	{
   		goto my_label2;
        	ret = ptr[2]; /* ERROR: Invalid memory access to already freed area */
	}
my_label:
            	for(i=0;i<5;i++)
            	{
	...
	if (str1 != NULL)
    	{
        	...
        	free(str_rev) ;
        	return str_rev; /* ERROR: Invalid memory access to already freed area */
    	}
    	else
    	{
       	 	return NULL;
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity:i Global double char pointer used after free
 *
 ***********************************************************************/
void invalid_memory_access_016_func_001()
{
	int flag = 0, i;
...
void invalid_memory_access_016_func_002()
{
	...
	strcpy(s,invalid_memory_access_016_doubleptr_gbl[0]); /* ERROR: Invalid memory access to already freed area */
}
...
void invalid_memory_access_016()
{
	...
	if (flag==0)
	{
		invalid_memory_access_016_func_001();
	}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity:i Global char pointer initialized in another function used in a while loop after free 
 *
 ***********************************************************************/
void invalid_memory_access_017_func_001()
{
...
void invalid_memory_access_017_func_003()
{
	...
	strcpy(s,invalid_memory_access_017_doubleptr_gbl); /* ERROR: Invalid memory access to already freed area */
}
...
void invalid_memory_access_017()
{
	...
	{
		invalid_memory_access_017_func_001();
	}
 ...
 *
 * Defect type: Invalid memory access to already freed area 
 * Complexity: Use of memory after memory was freed using alias to pointer 
 *
 ***********************************************************************/
void invalid_memory_access_018(void)
{
	char *a = calloc(32, sizeof(char));
	char *b = a;
	
	(void)sprintf(b, "%i", __LINE__);
	(void)printf("\t\tBuffer at: %p\tBuffer Contents: \"%s\"\n", a, a);
	free(b);
	(void)printf("\t\tFreed Buffer Contents: \"%s\"\n", a); /* ERROR: Invalid memory access to already freed area */
}
